...
> **Archived 2026-05-10.** This file contains the historical sections
> previously appended to `docs/PROJECT_STATUS.md`. They cover
> Windows-era paths, pre-migration framing, and prior strategy research
> that informed v11's current shape but is no longer the active source
> of truth.
>
> **Read this file for context, not for current state.** The active
> source of truth is the "Current state" section at the top of
> `docs/PROJECT_STATUS.md`. If something here conflicts with that
> section or with `CLAUDE.md`, the live sources win.
>
> Original sections preserved verbatim below (no edits, append-only
> per the original file's discipline).
