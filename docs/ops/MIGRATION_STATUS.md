...
| 4 | IBC + IB Gateway supervision | ✅ Done 2026-05-08 — IBC 3.23.0 at `~/ibc/`, `config.ini` from template w/ creds (0600), `gatewaystartmacos.sh` patched (port guard + `TWS_MAJOR_VRSN=10.46` + `IBC_PATH=~/ibc`), backup at `*.bak`. **2026-05-09:** added launchd supervisor `~/Library/LaunchAgents/com.ibc.gateway.plist` (KeepAlive=true, ThrottleInterval=300) after the 2026-05-08 23:45 ET silent overnight outage. **2026-05-10:** moved away from autorestart-token approach to a deterministic OS-level cron — `~/Library/LaunchAgents/com.nick.daily-restart.plist` fires `docs/agents/scripts/daily_restart.sh` at 01:15 ET daily. Gateway UI auto-logoff set to 01:00 ET, auto-restart turned OFF, IBC config `AutoRestartTime` + `ColdRestartTime` emptied. `com.ibc.gateway.plist` retained as mid-day safety net. Dry-run validated 2026-05-10 09:59 EDT — full cycle in 77s, no 2FA needed. See `docs/journal/2026-05-10_daily_restart_architecture.md`. |
