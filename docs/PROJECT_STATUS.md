...
| 2026-05-15 | `docs/journal/2026-05-15_gbpchf_sma50_cross_research.md` | GBPCHF 1-min MID/BID/ASK download complete (~3.6M rows in `tick_vault_data/fx/`). Pre-registered SMA50-cross research on 5-min: H1 (range-conditioned continuation) and H2 (trend-flip filter) both rejected. Real structural finding: persistent ~44-46% continuation rate (mild mean-reversion at cross), too small to clear 2-pip RT cost. Scripts + event dump retained for any future revisit. **Read this last for the current state of research progress** |
