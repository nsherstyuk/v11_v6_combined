"""Sequential XAUUSD 1-min bar downloader for MID, BID, ASK.

Mirrors `download_usdjpy_sequential.py` but adapted for gold:
  - XAUUSD is a CMDTY-type contract on SMART exchange (NOT Forex)
  - clientId=5 (v11 live=11, us-equities=2, GBPCHF=3, USDJPY=4)
  - START_DATE=2010-01-01 with empty-skip handling; IBKR head probe
    logs the actual depth at run start

Why this exists:
  The original XAUUSD ORB backtest data lived on Windows at
  C:\\nautilus0\\data\\1m_csv\\xauusd_1m_tick.csv and was never
  migrated to Mac. The live strategy runs on a positive-expectancy
  claim we cannot currently rerun. Downloading fresh from IBKR puts
  the data back on the Mac so v11/backtest/investigate_orb_xauusd.py
  can be rerun with multi-year depth.

Output:
    tick_vault_data/xauusd/XAUUSD_MIDPOINT.csv
    tick_vault_data/xauusd/XAUUSD_BID.csv
    tick_vault_data/xauusd/XAUUSD_ASK.csv

Run:
    python -m v11.backtest.download_xauusd_sequential
    python -m v11.backtest.download_xauusd_sequential --smoke
"""
...
START_DATE = datetime(2010, 6, 1)  # IBKR head probe 2026-05-18 returned
                                   # 2010-06-08 for MIDPOINT; start just
                                   # before so empty-skip catches the head.
...
OUTPUT_DIR = ROOT / "tick_vault_data" / "xauusd"
        ...
        if not bars:
            ...
            if consec_empty >= EMPTY_STREAK_LIMIT:
                print(f"    {chunk_end.date()}: {consec_empty} consec empty — "
                      f"skipping {EMPTY_SKIP_DAYS}d (pre-data zone)")
...
def _make_xauusd_contract():
    """XAUUSD on IBKR is a CMDTY contract on SMART, not Forex.
    Matches v11/config/live_config.py:XAUUSD_INSTRUMENT."""
    return Contract(
        symbol="XAUUSD",
        secType="CMDTY",
        exchange="SMART",
...
def run(data_types, start, end, out_dir):
    ...
    print("=" * 78)
    print(f"  {PAIR} sequential 1-min downloader (IBKR Cmdty)")
    ...
    contract = _make_xauusd_contract()
