"""Download AUDNZD 1-min bars (tick-aggregated, with bid/ask volume).

Same proven download_fx_universal.py logic used for EURUSD/XAUUSD/GBPUSD —
worker_per_proxy=1, 15s pause between weekly chunks, checkpoint every 4 weeks.

Writes to C:\\nautilus0\\data\\1m_csv_fresh\\audnzd_1m_tick.csv.
If that file exists from a partial run, resumes from the last timestamp.

Usage:
  python -m v11.backtest.download_audnzd
  python -m v11.backtest.download_audnzd --start 2018 --end 2026
"""
import sys
...
# AUDNZD is a 5-decimal pair like the other USD-base majors
PIPET_SIZE_REGISTRY.setdefault("AUDNZD", 1e-5)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            await asyncio.wait_for(
                download_range(symbol=symbol, start=start_date, end=end_date),
                timeout=300.0,
            )
            sys.stdout.write("reading... ")
            sys.stdout.flush()
            df_raw = read_tick_data(symbol=symbol, start=start_date, end=end_date, strict=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download AUDNZD 1-min bars (tick-aggregated).",
    )
